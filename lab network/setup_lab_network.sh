#!/bin/sh
# setup_lab_network.sh
# Creates an isolated Lab Wi-Fi network on OpenWrt / GL.iNet GL-BE3600
...
#
# Policy: Internet + LAN access (for Docker, VMs, internal testing tools).
#         Devices CAN talk to each other (no client isolation -- needed for testing).
#         No access to Guest or White-Hat zones.
...
clear
printf "\n"
printf "${BOLD}${MAGENTA}  +=========================================================+${R}\n"
printf "${BOLD}${MAGENTA}  |${R}                                                          ${BOLD}${MAGENTA}|${R}\n"
printf "${BOLD}${MAGENTA}  |${R}   ${BOLD}${WHITE}Lab Network Setup${R}  .  GL.iNet GL-BE3600              ${BOLD}${MAGENTA}|${R}\n"
printf "${BOLD}${MAGENTA}  |${R}   ${GRAY}Internet + LAN  .  Docker/VMs  .  Inter-device OK${R}      ${BOLD}${MAGENTA}|${R}\n"
printf "${BOLD}${MAGENTA}  |${R}                                                          ${BOLD}${MAGENTA}|${R}\n"
printf "${BOLD}${MAGENTA}  +=========================================================+${R}\n"
printf "\n"
...
printf "\n"
printf "  ${BOLD}${WHITE}Choose which Lab band to configure:${R}\n\n"
printf "  ${CYAN}1${R}  ${BOLD}2.4 GHz${R}\n"
printf "     ${GRAY}SSID   :${R}  amarof-Lab-2G-${DATE_TAG}\n"
printf "     ${GRAY}Subnet :${R}  192.168.40.0/24\n"
printf "     ${GRAY}Radio  :${R}  wifi0\n"
printf "\n"
printf "  ${CYAN}2${R}  ${BOLD}5 GHz${R}\n"
printf "     ${GRAY}SSID   :${R}  amarof-Lab-5G-${DATE_TAG}\n"
printf "     ${GRAY}Subnet :${R}  192.168.41.0/24\n"
...
while true; do
    ...
    case "$_band_choice" in
        1)
            BAND="2.4 GHz"
            NET_SSID_BASE="amarof-Lab-2G"
            NET_SSID="amarof-Lab-2G-${DATE_TAG}"
            NET_RADIO="wifi0"
            NET_IP="192.168.40.1"
            NET_BRIDGE="br-lab-2g"
            NET_IFACE="Lab2G"
            FW_ZONE="Lab2G"
            UCI_PFX="lab2g"
            break
        ...
        2)
            BAND="5 GHz"
            NET_SSID_BASE="amarof-Lab-5G"
            NET_SSID="amarof-Lab-5G-${DATE_TAG}"
            NET_RADIO="wifi1"
            NET_IP="192.168.41.1"
            NET_BRIDGE="br-lab-5g"
            NET_IFACE="Lab5G"
            FW_ZONE="Lab5G"
            UCI_PFX="lab5g"
            break
...
printf "  ${GRAY}Policy :${R}  Internet + LAN (Docker/VMs reachable)\n"
divider
...
# ---------------------------------------------------------------------
section "Scanning for existing $BAND Lab configuration"
printf "\n"
...
if [ "$FOUND_COUNT" -gt 0 ]; then
    ...
    while true; do
        ...
        case "$CHOICE" in
            1)
                printf "\n"
                info "Recreating $BAND Lab network from scratch ..."
                break
                ;;
            2)
                printf "\n"
...
# STEP 0b  Teardown
# ---------------------------------------------------------------------
section "Removing existing $BAND Lab configuration"
printf "\n"
...
ok "Client isolation ${BOLD}disabled${R}  ${GRAY}(lab devices CAN talk to each other -- needed for testing)${R}"
...
ok "Forward  $FW_ZONE  ->  lan  ${GRAY}(Docker/VMs/internal tools)${R}"
...
if ifstatus "$NET_IFACE" > /tmp/lab_status.json 2>&1; then
    UP=$(jsonfilter -i /tmp/lab_status.json -e '@["up"]' 2>/dev/null \
         || grep -o '"up":true' /tmp/lab_status.json || true)
    DEV=$(jsonfilter -i /tmp/lab_status.json -e '@["device"]' 2>/dev/null \
          || grep -o "\"device\":\"$NET_BRIDGE\"" /tmp/lab_status.json || true)
...
# Verify NO forwardings to Guest or White-Hat zones (Lab must not leak there)
...
if [ -z "$_leak" ]; then
    ok "Isolation verified  ${GRAY}(no forwarding from $FW_ZONE to Guest/White-Hat)${R}"
else
    err "ISOLATION LEAK -- found forwarding(s) to other isolated zones:"
...
if [ "$_iso" = "0" ]; then
    ok "Client isolation disabled  ${GRAY}(isolate=0 -- expected for Lab)${R}"
else
    warn "Client isolation is ON (isolate=$_iso) -- Lab devices won't see each other"
fi
...
if [ "$_ping_ok" = "1" ]; then
    ok "Router internet is up -- Lab clients will have internet via $NET_BRIDGE"
else
    err "Router cannot reach internet after 3 attempts"
    info "Check WAN connection -- Lab internet depends on router having internet"
    FAILURES=$((FAILURES + 1))
fi
...
if [ "$FAILURES" -eq 0 ]; then
    ...
    printf "${BOLD}${GREEN}  |${R}  ${GREEN}$BAND Lab network is live and ready to use${R}             ${BOLD}${GREEN}|${R}\n"
    printf "${BOLD}${GREEN}  +=========================================================+${R}\n"
else
    printf "${BOLD}${RED}  +=========================================================+${R}\n"
...
printf "  ${GRAY}Forwarding  :${R}  $FW_ZONE -> wan  +  $FW_ZONE -> lan\n"
printf "  ${GRAY}IPv6        :${R}  ${GREEN}disabled${R}\n"
printf "  ${GRAY}Isolation   :${R}  ${YELLOW}disabled${R}  ${GRAY}(Lab devices can talk to each other)${R}\n"
printf "\n"
divider
...
printf "  ${CYAN}5${R}  Try pinging  ${BOLD}192.168.30.1${R} (Guest)  -- should ${RED}fail${R}\n"
printf "  ${CYAN}6${R}  Lab devices should be able to ping each other\n"
printf "\n"
