#!/bin/sh
# setup_white_hat_network.sh
# Creates an isolated White-Hat Wi-Fi network on OpenWrt / GL.iNet GL-BE3600
...
#
# Policy: Internet + LAN + Lab access (used for controlled security/pentest scenarios).
#         Devices CAN talk to each other (no isolation -- needed for scanning).
#         CANNOT access the Guest zone.
#         Forwarding rules to Lab2G/Lab5G will be created even if Lab zones don't
#         exist yet -- they become active automatically once Lab is set up.
...
printf "\n"
printf "${BOLD}${RED}  +=========================================================+${R}\n"
printf "${BOLD}${RED}  |${R}                                                          ${BOLD}${RED}|${R}\n"
printf "${BOLD}${RED}  |${R}   ${BOLD}${WHITE}White-Hat Network Setup${R}  .  GL.iNet GL-BE3600        ${BOLD}${RED}|${R}\n"
printf "${BOLD}${RED}  |${R}   ${GRAY}Controlled pentest lab  .  Internet + LAN + Lab access${R}  ${BOLD}${RED}|${R}\n"
printf "${BOLD}${RED}  |${R}                                                          ${BOLD}${RED}|${R}\n"
printf "${BOLD}${RED}  +=========================================================+${R}\n"
printf "\n"
...
printf "\n"
printf "  ${BOLD}${WHITE}Choose which White-Hat band to configure:${R}\n\n"
printf "  ${CYAN}1${R}  ${BOLD}2.4 GHz${R}\n"
printf "     ${GRAY}SSID   :${R}  amarof-WhiteHat-2G-${DATE_TAG}\n"
printf "     ${GRAY}Subnet :${R}  192.168.50.0/24\n"
printf "     ${GRAY}Radio  :${R}  wifi0\n"
printf "\n"
printf "  ${CYAN}2${R}  ${BOLD}5 GHz${R}\n"
printf "     ${GRAY}SSID   :${R}  amarof-WhiteHat-5G-${DATE_TAG}\n"
printf "     ${GRAY}Subnet :${R}  192.168.51.0/24\n"
...
while true; do
    ...
    case "$_band_choice" in
        1)
            BAND="2.4 GHz"
            NET_SSID_BASE="amarof-WhiteHat-2G"
            NET_SSID="amarof-WhiteHat-2G-${DATE_TAG}"
            NET_RADIO="wifi0"
            NET_IP="192.168.50.1"
            NET_BRIDGE="br-wh-2g"
            NET_IFACE="WhiteHat2G"
            FW_ZONE="WhiteHat2G"
            UCI_PFX="wh2g"
            break
            ;;
        2)
            BAND="5 GHz"
            NET_SSID_BASE="amarof-WhiteHat-5G"
            NET_SSID="amarof-WhiteHat-5G-${DATE_TAG}"
            NET_RADIO="wifi1"
            NET_IP="192.168.51.1"
            NET_BRIDGE="br-wh-5g"
            NET_IFACE="WhiteHat5G"
            FW_ZONE="WhiteHat5G"
            UCI_PFX="wh5g"
            break
            ;;
...
printf "  ${GRAY}Policy :${R}  Internet + LAN + Lab2G + Lab5G  (NOT Guest)\n"
divider
...
# ---------------------------------------------------------------------
section "Scanning for existing $BAND White-Hat configuration"
printf "\n"
...
if [ "$FOUND_COUNT" -gt 0 ]; then
    ...
    while true; do
        ...
        case "$CHOICE" in
            1)
                printf "\n"
                info "Recreating $BAND White-Hat network from scratch ..."
                break
                ;;
            2)
                printf "\n"
...
# STEP 0b  Teardown
# ---------------------------------------------------------------------
section "Removing existing $BAND White-Hat configuration"
printf "\n"
...
ok "IPv6 prefix delegation ${BOLD}disabled${R}"
...
uci set wireless."${UCI_PFX}_wifi".isolate='0'

ok "SSID  ${BOLD}$NET_SSID${R}  on radio ${BOLD}$NET_RADIO${R}  ($BAND)  ->  network $NET_IFACE"
ok "Client isolation ${BOLD}disabled${R}  ${GRAY}(security testing needs visibility between devices)${R}"
...
ok "Forward  $FW_ZONE  ->  wan  ${GRAY}(internet)${R}"

uci set firewall."${UCI_PFX}_to_lan"=forwarding
uci set firewall."${UCI_PFX}_to_lan".src="$FW_ZONE"
uci set firewall."${UCI_PFX}_to_lan".dest='lan'
ok "Forward  $FW_ZONE  ->  lan  ${GRAY}(trusted LAN -- pentest targets)${R}"

uci set firewall."${UCI_PFX}_to_lab2g"=forwarding
uci set firewall."${UCI_PFX}_to_lab2g".src="$FW_ZONE"
uci set firewall."${UCI_PFX}_to_lab2g".dest='Lab2G'
ok "Forward  $FW_ZONE  ->  Lab2G  ${GRAY}(active once Lab 2G is set up)${R}"

uci set firewall."${UCI_PFX}_to_lab5g"=forwarding
uci set firewall."${UCI_PFX}_to_lab5g".src="$FW_ZONE"
uci set firewall."${UCI_PFX}_to_lab5g".dest='Lab5G'
ok "Forward  $FW_ZONE  ->  Lab5G  ${GRAY}(active once Lab 5G is set up)${R}"
...
if ifstatus "$NET_IFACE" > /tmp/wh_status.json 2>&1; then
    UP=$(jsonfilter -i /tmp/wh_status.json -e '@["up"]' 2>/dev/null \
         || grep -o '"up":true' /tmp/wh_status.json || true)
    DEV=$(jsonfilter -i /tmp/wh_status.json -e '@["device"]' 2>/dev/null \
          || grep -o "\"device\":\"$NET_BRIDGE\"" /tmp/wh_status.json || true)
...
if [ -f /tmp/dhcp.leases ]; then
    ...
    if [ -n "$_good" ]; then
        ok "Active White-Hat leases found:"
...
_check "uci show firewall.${UCI_PFX}_to_lan"   \
    "Forwarding  $FW_ZONE -> lan"               \
    "Missing forwarding $FW_ZONE -> lan"

_check "uci show firewall.${UCI_PFX}_to_lab2g"   \
    "Forwarding  $FW_ZONE -> Lab2G"               \
    "Missing forwarding $FW_ZONE -> Lab2G"

_check "uci show firewall.${UCI_PFX}_to_lab5g"   \
    "Forwarding  $FW_ZONE -> Lab5G"               \
    "Missing forwarding $FW_ZONE -> Lab5G"
...
# Verify NO forwardings to Guest zone (White-Hat must not leak to Guest)
...
if [ -z "$_leak" ]; then
    ok "Isolation verified  ${GRAY}(no forwarding from $FW_ZONE to Guest)${R}"
else
    err "ISOLATION LEAK -- found forwarding(s) to Guest:"
    echo "$_leak" | while read -r _line; do printf "       ${RED}%s${R}\n" "$_line"; done
    FAILURES=$((FAILURES + 1))
fi

# Check if Lab zones exist -- informational only (not a failure)
if uci show firewall 2>/dev/null | grep -q "name='Lab2G'"; then
    ok "Lab2G zone present  ${GRAY}(forwarding to Lab2G is active)${R}"
else
    warn "Lab2G zone not present -- forwarding rule exists but inactive until Lab 2G is set up"
fi
if uci show firewall 2>/dev/null | grep -q "name='Lab5G'"; then
    ok "Lab5G zone present  ${GRAY}(forwarding to Lab5G is active)${R}"
else
    warn "Lab5G zone not present -- forwarding rule exists but inactive until Lab 5G is set up"
fi

_iso=$(uci get wireless."${UCI_PFX}_wifi".isolate 2>/dev/null || echo "0")
if [ "$_iso" = "0" ]; then
    ok "Client isolation disabled  ${GRAY}(isolate=0 -- expected for White-Hat)${R}"
else
    warn "Client isolation is ON (isolate=$_iso) -- pentest devices won't see each other"
fi
...
if [ "$_ping_ok" = "1" ]; then
    ok "Router internet is up -- White-Hat clients will have internet via $NET_BRIDGE"
else
    err "Router cannot reach internet after 3 attempts"
    info "Check WAN connection -- White-Hat internet depends on router having internet"
    FAILURES=$((FAILURES + 1))
fi
...
if [ "$FAILURES" -eq 0 ]; then
    ...
    printf "${BOLD}${GREEN}  |${R}  ${GREEN}$BAND White-Hat network is live and ready to use${R}       ${BOLD}${GREEN}|${R}\n"
    printf "${BOLD}${GREEN}  +=========================================================+${R}\n"
else
    printf "${BOLD}${RED}  +=========================================================+${R}\n"
...
printf "  ${GRAY}Forwarding  :${R}  $FW_ZONE -> wan, lan, Lab2G, Lab5G\n"
printf "  ${GRAY}IPv6        :${R}  ${GREEN}disabled${R}\n"
printf "  ${GRAY}Isolation   :${R}  ${YELLOW}disabled${R}  ${GRAY}(pentest devices can scan each other)${R}\n"
printf "\n"
divider
...
printf "  ${CYAN}4${R}  Try pinging  ${BOLD}192.168.8.1${R} (LAN gateway)  -- should ${GREEN}succeed${R}\n"
printf "  ${CYAN}5${R}  Try pinging  ${BOLD}192.168.40.1${R} (Lab 2G)  -- should ${GREEN}succeed${R} (if Lab 2G exists)\n"
printf "  ${CYAN}6${R}  Try pinging  ${BOLD}192.168.41.1${R} (Lab 5G)  -- should ${GREEN}succeed${R} (if Lab 5G exists)\n"
printf "  ${CYAN}7${R}  Try pinging  ${BOLD}192.168.30.1${R} (Guest)   -- should ${RED}fail${R}\n"
printf "\n"
